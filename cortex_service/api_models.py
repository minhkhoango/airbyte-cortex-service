...
class ChunkingStrategyParams(BaseModel):
    ...
    chunk_size: int = Field(1000, description="Max characters per chunk for fixed_size.")
    chunk_overlap: int = Field(100, description="Overlap for fixed_size.")
    min_chunk_size: int = Field(50, description="Min characters for paragraph chunking.")

class ChunkingStrategy(BaseModel):
    """Defines the chunking strategy to be used."""
    name: Literal["fixed_size", "paragraph"] = Field(..., description="The name of the strategy.")
...
class ChunkValidation(BaseModel):
    ...
    similarity_with_next_chunk: Optional[float] = Field(
        None, description="Cosine similarity score with the following chunk. Null for the last chunk."
    )
